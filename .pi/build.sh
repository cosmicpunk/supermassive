...
# docker buildx build --tag supermassive --file .pi/Dockerfile --platform linux/arm64 --cache-from supermassive:cache --cache-to supermassive:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar supermassive
...
sudo bash -c "echo supermassive > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/supermassive.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/supermassive.img
